'use strict';


// Unprefixed

const $ = exports.$ = new Array(0x100);


$[0] = ['NOP', (cpu) => {
    cpu.pc += 1;

    return 4;
}];

$[0x1] = ['LD BC,d16', (cpu, mmu) => {
    cpu.bc = mmu.readWord(cpu.pc + 1);
    cpu.pc += 3;

    return 12;
}];

$[0x4] = ['INC B', (cpu) => {
    cpu.b++;

    cpu.f &= ~0xe0;
    if (cpu.b == 0) cpu.f |= 1 << 7;
    else if (cpu.b == 0x10) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0x5] = ['DEC B', (cpu) => {
    cpu.b--;

    cpu.f &= ~0xe0;
    cpu.f |= 1 << 6;
    if (cpu.b == 0) cpu.f |= 1 << 7;
    if ((cpu.b & 0xf) < 1) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0x6] = ['LD B,d8', (cpu, mmu) => {
    cpu.b = mmu.readByte(cpu.pc + 1);
    cpu.pc += 2;

    return 8;
}];

$[0xb] = ['DEC BC', (cpu) => {
    cpu.bc--;
    cpu.pc += 1;

    return 8;
}];

$[0xc] = ['INC C', (cpu) => {
    cpu.c++;

    cpu.f &= ~0xe0;
    if (cpu.c == 0) cpu.f |= 1 << 7;
    else if (cpu.c == 0x10) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0xd] = ['DEC C', (cpu) => {
    cpu.c--;

    cpu.f &= ~0xe0;
    cpu.f |= 1 << 6;
    if (cpu.c == 0) cpu.f |= 1 << 7;
    if ((cpu.c & 0xf) < 1) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0xe] = ['LD C,d8', (cpu, mmu) => {
    cpu.c = mmu.readByte(cpu.pc + 1);
    cpu.pc += 2;

    return 8;
}];

$[0x11] = ['LD DE,d16', (cpu, mmu) => {
    cpu.de = mmu.readWord(cpu.pc + 1);
    cpu.pc += 3;

    return 12;
}];

$[0x13] = ['INC DE', (cpu) => {
    cpu.de++;
    cpu.pc += 1;

    return 8;
}];

$[0x15] = ['DEC D', (cpu) => {
    cpu.d--;

    cpu.f &= ~0xe0;
    cpu.f |= 1 << 6;
    if (cpu.d == 0) cpu.f |= 1 << 7;
    if ((cpu.d & 0xf) < 1) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0x16] = ['LD D,d8', (cpu, mmu) => {
    cpu.d = mmu.readByte(cpu.pc + 1);
    cpu.pc += 2;

    return 8;
}];

$[0x17] = ['RLA', (cpu) => {
    let v;
    cpu.a = v = cpu.a << 1;

    cpu.f &= ~0xe0;
    if (cpu.f >> 4) cpu.a |= 1;
    if (v >> 8) cpu.f |= 1 << 4;

    cpu.pc += 1;

    return 4;
}];

$[0x18] = ['JR r8', (cpu, mmu) => {
    const r8 = mmu.readByte(cpu.pc + 1);
    const offset = r8 & 0x80 ? -((0xff & ~r8) + 1) : r8;

    cpu.pc += 2 + offset;

    return 12;
}];

$[0x19] = ['ADD HL,DE', (cpu, mmu) => {
    let v;
    cpu.hl = v = cpu.hl + cpu.de;

    cpu.f &= ~0x7f;
    if ((~cpu.hl & 0xff) == 0) cpu.f |= 1 << 5;
    if (v > 0xff) cpu.f |= 1 << 4;

    cpu.pc += 1;

    return 8;
}];

$[0x1a] = ['LD A,(DE)', (cpu, mmu) => {
    cpu.a = mmu.readByte(cpu.de);
    cpu.pc += 1;

    return 8;
}];

$[0x1d] = ['DEC E', (cpu) => {
    cpu.e--;

    cpu.f &= ~0xa0;
    if (cpu.e == 0) cpu.f |= 1 << 7;
    cpu.f |= 1 << 6;
    if ((~cpu.e & 0xf) == 0) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0x1e] = ['LD E,d8', (cpu, mmu) => {
    cpu.e = mmu.readByte(cpu.pc + 1);
    cpu.pc += 2;

    return 8;
}];

$[0x20] = ['JR NZ,r8', (cpu, mmu) => {
    if (cpu.f >> 7 == 0) {
        const n = mmu.readByte(cpu.pc + 1);
        const offset = n & 0x80 ? -((0xff & ~n) + 1) : n;

        cpu.pc += 2 + offset;

        return 12;
    }
    cpu.pc += 2;

    return 8;
}];

$[0x21] = ['LD HL,d16', (cpu, mmu) => {
    cpu.hl = mmu.readWord(cpu.pc + 1);
    cpu.pc += 3;

    return 12;
}];

$[0x22] = ['LD (HL+),A', (cpu, mmu) => {
    mmu.writeWord(cpu.hl++, cpu.a);
    cpu.pc += 1;

    return 8;
}];

$[0x23] = ['INC HL', (cpu) => {
    cpu.hl++;
    cpu.pc += 1;

    return 8;
}];

$[0x24] = ['INC H', (cpu) => {
    let v;
    cpu.h = v = cpu.h + 1;

    cpu.f &= ~0xe0;
    if (cpu.h == 0) cpu.f |= 1 << 7;
    if (v == 0xf) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0x28] = ['JR Z,r8', (cpu, mmu) => {
    if (cpu.f >> 7 == 1) {
        const n = mmu.readByte(cpu.pc + 1);
        const offset = n & 0x80 ? -((0xff & ~n) + 1) : n;

        cpu.pc += 2 + offset;

        return 12;
    }
    cpu.pc += 2;

    return 8;
}];

$[0x30] = ['JR NC,r8', (cpu, mmu) => {
    if (cpu.f >> 4 & 1 == 0) {
        const n = mmu.readByte(cpu.pc + 1);
        const offset = n & 0x80 ? -((0xff & ~n) + 1) : n;

        cpu.pc += 2 + offset;

        return 12;
    }
    cpu.pc += 2;

    return 8;
}];

$[0x2a] = ['LD A,(HL+)', (cpu, mmu) => {
    cpu.a = mmu.readByte(cpu.hl++);
    cpu.pc += 1;

    return 8;
}];

$[0x2e] = ['LD L,d8', (cpu, mmu) => {
    cpu.l = mmu.readByte(cpu.pc + 1);
    cpu.pc += 2;

    return 8;
}];

$[0x2f] = ['CPL', (cpu) => {
    cpu.a ^= 0xff;
    cpu.pc += 1;

    return 4;
}];

$[0x31] = ['LD SP,d16', (cpu, mmu) => {
    cpu.sp = mmu.readWord(cpu.pc + 1);
    cpu.pc += 3;

    return 12;
}];

$[0x32] = ['LD (HL-),A', (cpu, mmu) => {
    mmu.writeByte(cpu.hl--, cpu.a);
    cpu.pc += 1;

    return 8;
}];

$[0x34] = ['INC (HL)', (cpu, mmu) => {
    const v = mmu.writeByte(cpu.hl, mmu.readByte(cpu.hl) + 1);

    cpu.f &= ~0xe0;
    if (v == 0) cpu.f |= 1 << 7;
    else if (v == 0x10) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 12;
}];

$[0x36] = ['LD (HL),d8', (cpu, mmu) => {
    mmu.writeByte(cpu.hl, mmu.readByte(cpu.pc + 1));
    cpu.pc += 2;

    return 12;
}];

$[0x3c] = ['INC A', (cpu) => {
    cpu.a++;

    cpu.f &= ~0xe0;
    if (cpu.a == 0) cpu.f |= 1 << 7;
    else if (cpu.a == 0x10) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0x3d] = ['DEC A', (cpu) => {
    cpu.a--;

    cpu.f &= ~0xa0;
    if (cpu.a == 0) cpu.f |= 1 << 7;
    cpu.f |= 1 << 6;
    if ((~cpu.a & 0xf) == 0) cpu.f |= 1 << 5;

    cpu.pc += 1;

    return 4;
}];

$[0x3e] = ['LD A,d8', (cpu, mmu) => {
    cpu.a = mmu.readByte(cpu.pc + 1);
    cpu.pc += 2;

    return 8;
}];

$[0x47] = ['LD B,A', (cpu) => {
    cpu.b = cpu.a;
    cpu.pc += 1;

    return 4;
}];

$[0x4f] = ['LD C,A', (cpu) => {
    cpu.c = cpu.a;
    cpu.pc += 1;

    return 4;
}];

$[0x56] = ['LD D,(HL)', (cpu, mmu) => {
    cpu.d = mmu.readByte(cpu.hl);
    cpu.pc += 1;

    return 8;
}];

$[0x57] = ['LD D,A', (cpu) => {
    cpu.d = cpu.a;
    cpu.pc += 1;

    return 4;
}];

$[0x5e] = ['LD E,(HL)', (cpu, mmu) => {
    cpu.e = mmu.readByte(cpu.hl);
    cpu.pc += 1;

    return 8;
}];

$[0x5f] = ['LD E,A', (cpu) => {
    cpu.e = cpu.a;
    cpu.pc += 1;

    return 4;
}];

$[0x67] = ['LD H,A', (cpu) => {
    cpu.h = cpu.a;
    cpu.pc += 1;

    return 4;
}];

$[0x77] = ['LD (HL),A', (cpu, mmu) => {
    mmu.writeByte(cpu.hl, cpu.a);
    cpu.pc += 1;

    return 8;
}];

$[0x78] = ['LD A,B', (cpu) => {
    cpu.a = cpu.b;
    cpu.pc += 1;

    return 4;
}];

$[0x79] = ['LD A,C', (cpu) => {
    cpu.a = cpu.c;
    cpu.pc += 1;

    return 4;
}];

$[0x7b] = ['LD A,E', (cpu) => {
    cpu.a = cpu.e;
    cpu.pc += 1;

    return 4;
}];

$[0x7c] = ['LD A,H', (cpu) => {
    cpu.a = cpu.h;
    cpu.pc += 1;

    return 4;
}];

$[0x7d] = ['LD A,L', (cpu, mmu) => {
    cpu.a = cpu.l;
    cpu.pc += 1;

    return 4;
}];

$[0x86] = ['ADD A,(HL)', (cpu, mmu) => {
    let v;
    cpu.a = v = cpu.a + mmu.readByte(cpu.hl);

    cpu.f = 0;
    if (cpu.a == 0) cpu.f |= 1 << 7;
    if ((~cpu.a & 0xf) == 0) cpu.f |= 1 << 5;
    if (v > 0xff) cpu.f |= 1 << 4;

    cpu.pc += 1;

    return 8;
}];

$[0x87] = ['ADD A,A', (cpu, mmu) => {
    let v;
    cpu.a = v = cpu.a + cpu.a;

    cpu.f = 0;
    if (cpu.a == 0) cpu.f |= 1 << 7;
    if ((~cpu.a & 0xf) == 0) cpu.f |= 1 << 5;
    if (v > 0xff) cpu.f |= 1 << 4;

    cpu.pc += 1;

    return 4;
}];

$[0x90] = ['SUB B', (cpu) => {
    let v;
    cpu.a = v = cpu.a - cpu.b;

    cpu.f = 0;
    if (cpu.a == 0) cpu.f |= 1 << 7;
    cpu.f |= 1 << 6;
    if ((~cpu.a & 0xf) == 0) cpu.f |= 1 << 5;
    if (v < 0) cpu.f |= 1 << 4;

    cpu.pc += 1;

    return 4;
}];

$[0xa1] = ['AND C', (cpu) => {
    cpu.a &= cpu.c;

    cpu.f = 0x20;
    if (cpu.a == 0) cpu.f |= 1 << 7;

    cpu.pc += 1;

    return 4;
}];

$[0xa7] = ['AND A', (cpu) => {
    cpu.a &= cpu.a;

    cpu.f = 0x20;
    if (cpu.a == 0) cpu.f |= 1 << 7;

    cpu.pc += 1;

    return 4;
}];

$[0xaf] = ['XOR A', (cpu) => {
    cpu.a ^= cpu.a;

    cpu.f = cpu.a == 0 ? 0x80 : 0;
    cpu.pc += 1;

    return 4;
}];


$[0xa9] = ['XOR C', (cpu) => {
    cpu.a ^= cpu.c;

    cpu.f = cpu.a == 0 ? 0x80 : 0;
    cpu.pc += 1;

    return 4;
}];

$[0xb0] = ['OR B', (cpu) => {
    cpu.a |= cpu.b;

    cpu.f = 0;
    if (cpu.a == 0) cpu.f |= 1 << 7;

    cpu.pc += 1;

    return 4;
}];

$[0xb1] = ['OR C', (cpu) => {
    cpu.a |= cpu.c;

    cpu.f = 0;
    if (cpu.a == 0) cpu.f |= 1 << 7;

    cpu.pc += 1;

    return 4;
}];

$[0xbe] = ['CP (HL)', (cpu, mmu) => {
    const n = mmu.readByte(cpu.hl);
    const cp = cpu.a - n;

    if (cp == 0) cpu.f |= 1 << 7;
    else cpu.f &= ~(1 << 7);
    cpu.f |= 1 << 6;
    if (cp >> 4 & 1) cpu.f |= 1 << 5;
    else cpu.f &= ~(1 << 5);
    if (cpu.a < n) cpu.f |= 1 << 4;
    else cpu.f &= ~(1 << 4);

    cpu.pc += 1;

    return 8;
}];

$[0xc0] = ['RET NZ', (cpu, mmu) => {
    if (cpu.f >> 7 == 0) {
        cpu.pc = mmu.readWord(cpu.sp);
        cpu.sp += 2;

        return 20;
    }
    cpu.pc += 1;

    return 8;
}];


$[0xc1] = ['POP BC', (cpu, mmu) => {
    cpu.bc = mmu.readWord(cpu.sp);
    cpu.sp += 2;
    cpu.pc += 1;

    return 12;
}];

$[0xc3] = ['JP a16', (cpu, mmu) => {
    cpu.pc = mmu.readWord(cpu.pc + 1);

    return 16;
}];

$[0xc5] = ['PUSH BC', (cpu, mmu) => {
    mmu.writeWord(cpu.sp -= 2, cpu.bc);
    cpu.pc += 1;

    return 16;
}];

$[0xc8] = ['RET Z', (cpu, mmu) => {
    if (cpu.f >> 7) {
        cpu.pc = mmu.readWord(cpu.sp);
        cpu.sp += 2;

        return 20;
    }
    cpu.pc += 1;

    return 8;
}];

$[0xc9] = ['RET', (cpu, mmu) => {
    cpu.pc = mmu.readWord(cpu.sp);
    cpu.sp += 2;

    return 16;
}];

$[0xcd] = ['CALL a16', (cpu, mmu) => {
    mmu.writeWord(cpu.sp -= 2, cpu.pc + 3);
    cpu.pc = mmu.readWord(cpu.pc + 1);

    return 24;
}];

$[0xd1] = ['POP DE', (cpu, mmu) => {
    cpu.de = mmu.readWord(cpu.sp);
    cpu.sp += 2;
    cpu.pc += 1;

    return 12;
}];

$[0xd5] = ['PUSH DE', (cpu, mmu) => {
    mmu.writeWord(cpu.sp -= 2, cpu.de);
    cpu.pc += 1;

    return 16;
}];

// TODO(dmacosta) confirm below

$[0xd9] = ['RETI', (cpu, mmu) => {
    cpu.pc = mmu.readWord(cpu.sp);
    cpu.sp += 2;
    cpu.ime = 1;

    return 16;
}];

$[0xe0] = ['LDH (a8),A', (cpu, mmu) => {
    mmu.writeByte(0xff00 + mmu.readByte(cpu.pc + 1), cpu.a);
    cpu.pc += 2;

    return 12;
}];

$[0xe1] = ['POP HL', (cpu, mmu) => {
    cpu.hl = mmu.readWord(cpu.sp);
    cpu.sp += 2;
    cpu.pc += 1;

    return 12;
}];

$[0xe2] = ['LD (C),A', (cpu, mmu) => {
    mmu.writeByte(0xff00 + cpu.c, cpu.a);
    cpu.pc += 1;

    return 8;
}];

$[0xe5] = ['PUSH HL', (cpu, mmu) => {
    mmu.writeWord(cpu.sp -= 2, cpu.hl);
    cpu.pc += 1;

    return 16;
}];

$[0xe6] = ['AND d8', (cpu, mmu) => {
    cpu.a &= mmu.readByte(cpu.pc + 1);

    cpu.f = 0x20;
    if (cpu.a == 0) cpu.f |= 1 << 7;

    cpu.pc += 2;

    return 8;
}];

$[0xe9] = ['JP (HL)', (cpu, mmu) => {
    cpu.pc = cpu.hl;

    return 4;
}];

$[0xea] = ['LD (a16),A', (cpu, mmu) => {
    mmu.writeByte(mmu.readWord(cpu.pc + 1), cpu.a);
    cpu.pc += 3;

    return 16;
}];

$[0xef] = ['RST 28H', (cpu, mmu) => {
    mmu.writeWord(cpu.sp -= 2, cpu.pc);
    cpu.pc = 0x28;

    return 16;
}];

$[0xf0] = ['LDH A,(a8)', (cpu, mmu) => {
    cpu.a = mmu.readByte(0xff00 + mmu.readByte(cpu.pc + 1));
    cpu.pc += 2;

    return 12;
}];

$[0xf1] = ['POP AF', (cpu, mmu) => {
    cpu.af = mmu.readWord(cpu.sp);
    cpu.sp += 2;
    cpu.pc += 1;

    return 12;
}];

$[0xfa] = ['LD A,(a16)', (cpu, mmu) => {
    cpu.a = mmu.readByte(mmu.readWord(cpu.pc + 1));
    cpu.pc += 3;

    return 16;
}];

// TODO(dmacosta) fix below

$[0xf3] = ['DI', (cpu) => {
    cpu.ime = 0;
    cpu.pc += 1;

    return 4;
}];

$[0xf5] = ['PUSH AF', (cpu, mmu) => {
    mmu.writeWord(cpu.sp -= 2, cpu.af);
    cpu.pc += 1;

    return 16;
}];

// TODO(dmacosta) same as DI

$[0xfb] = ['EI', (cpu) => {
    cpu.ime = 1;
    cpu.pc += 1;

    return 4;
}];

$[0xfe] = ['CP d8', (cpu, mmu) => {
    const n = mmu.readByte(cpu.pc + 1);
    const cp = cpu.a - n;

    if (cp == 0) cpu.f |= 1 << 7;
    else cpu.f &= ~(1 << 7);
    cpu.f |= 1 << 6;
    if (cp >> 4 & 1) cpu.f |= 1 << 5;
    else cpu.f &= ~(1 << 5);
    if (cpu.a < n) cpu.f |= 1 << 4;
    else cpu.f &= ~(1 << 4);

    cpu.pc += 2;

    return 8;
}];

// CB Prefixed

const $cb = $[0xcb] = new Array(0x100);

$cb[0x11] = ['RL C', (cpu) => {
    let v;
    cpu.c = v = cpu.c << 1;

    cpu.f = 0;
    if (cpu.c == 0) cpu.f |= 1 << 7;
    if (v >> 8 == 1) cpu.f |= 1 << 4;

    cpu.pc += 2;

    return 8;
}];

$cb[0x7c] = ['BIT 7,H', (cpu) => {
    if (cpu.h >> 7 == 0) cpu.f |= 1 << 7;
    else cpu.f &= ~(1 << 7);
    cpu.f &= ~(1 << 6);
    cpu.f |= 1 << 5;

    cpu.pc += 2;

    return 8;
}];

$cb[0x37] = ['SWAP A', (cpu) => {
    cpu.a = cpu.a << 4 | cpu.a >> 4;

    cpu.f = 0;
    if (cpu.a == 0) cpu.f |= 1 << 7;

    cpu.pc += 2;

    return 8;
}];
