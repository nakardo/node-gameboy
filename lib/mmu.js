...
class Mmu extends Serializable({ exclude }) {
    ...
    readByte (addr) {
        ...
        switch (addr >> 12) {
            ...
            case 0xc: case 0xd:
                const bank = (addr >> 12) == 0xc ? 0 : this._wramBank;
                return this._wram[bank][addr & 0x1fff];
            case 0xe: case 0xf:
    ...
    writeByte (addr, val) {
        ...
        switch (addr >> 12) {
            ...
            case 0xc: case 0xd:
                const bank = (addr >> 12) == 0xc ? 0 : this._wramBank;
                return this._wram[bank][addr & 0x1fff] = val;
            case 0xe: case 0xf:
                if (addr == IF) return this.if = val;
                if (addr == IE) return this.ie = val;

                switch (addr) {
                    case VBK: this.video.ramBank = val & 1;
                    case DMA: this.video.transfer(this, val);
                    case SVBK:
                        const bank = val & 0x7;
