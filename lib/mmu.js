...
const { SVBK, IF, IE } = require('./registers');
...
class Mmu extends Serializable({ exclude }) {
    constructor (bios = []) {
        ...
        this._wramBank = 1;
        this._wram = new Array(8).fill().map(() => new Uint8Array(0x1000));
    ...
    writeByte (addr, val) {
        ...
        switch (addr >> 12) {
            ...
            case 0xe: case 0xf:
                ...
                if (addr > 0xfeff) {
                    if (addr == SVBK) return this._wramBank = (val & 0x7) | 1;
                    switch (addr & 0xff) {
                        case 0:
