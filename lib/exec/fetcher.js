...
var indirect = function (cpu, operand, pos) {
    ...
    return function (source) {
        ...
        if (source !== undefined) switch (source.operand) {
            ...
            default:
                debug('%s operand cannot be updated. stop',
                    operand.toUpperCase());
                process.exit(1);
        }
...
var fetch = function (cpu, operands, pos) {
    ...
    var isIndirect = /\(.+\)/.test(operand);

    operand = operand.match(/[\w\d+-]+/)[0].toLowerCase();

    var fetch = isIndirect
        ? indirect(cpu, operand, pos)
        : immediate.bind(this, cpu, operand);

    fetch.operand = /[\w\d]+/.exec(operand)[0]; // strip HL+, HL-
