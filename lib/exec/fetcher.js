...
var indirect = function (cpu, operand, pos) {
    ...
    return function (value) {
        ...
        if (value !== undefined) switch (operand) {
            case 'a8': case 'C':
                return cpu.mmu.writeByte(reference, value);
        ...
        switch (operand) {
            case 'a8': case 'C':
                return cpu.mmu.readByte(reference);
            case 'a16':
                return cpu.mmu.readWord(reference);
            case 'BC': case 'DE': case 'HL': case 'HL+': case 'HL-':
                return indirect;
            default:
                debug('%s operand cannot be updated. stop', operand);
                process.exit(1);
...
var fetch = function (cpu, operands, pos) {
    ...
    var indirection = operand.match(/\(([A-Za-z+-]+)\)/);

    var fetch = indirection
        ? indirect(cpu, indirection[1], pos)
        : immediate.bind(this, cpu, operand);

    fetch.operand = operand.match(/(\w+)/)[0].toLowerCase();
    fetch.indirect = indirection !== null;
