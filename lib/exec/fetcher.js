...
var immediate = function (cpu, pc, operand) {
    ...
    switch (operand) {
        ...
        case 'z':  value = bitutil.test(cpu.f, 7); break;
        case 'nz': value = bitutil.test(cpu.f, 7) === 1 ? 0 : 1; break;
        case 'nc': value = bitutil.test(cpu.f, 4) === 1 ? 0 : 1; break;

        // Immediate data

        case 'r8':
            value = bitutil.signed(cpu.mmu.readByte(next), 0xFF)
            break;
        case 'd8': case 'a8':
            value = cpu.mmu.readByte(next);
            break;
...
var indirect = function (cpu, pc, operand, pos) {
    ...
    return function (source) {
        ...
        switch (operand) {
            ...
            default: return cpu.mmu.readByte(reference);
        }
    }
}
