...
before(function (cpu, spec) {
    ...
    if (cpu.pc >= 0x0086) {
        console.log('pc=%s, a=%s b=%s d=%s', cpu.pc.toString(16), cpu.a, cpu.b, cpu.d);
        console.log('hl=%s de=%s', cpu.hl.toString(16), cpu.de.toString(16));
    }
...
inst('dec', function (cpu, op1) {
    ...
    var updated = op1.indirect
        ? op1(value)
        : cpu[op1.operand] = value
        ;

    var updateFlags = [ 'a', 'b', 'c', 'd', 'e', 'h', 'l'].indexOf(op1.operand) >= 0 ||
        (op1.operand === 'hl' && op1.indirect);

    if (updateFlags) cpu.flag('z', updated === 0);
...
inst('sub', function (cpu, op1) {
    return cpu.flag('z', (cpu.a -= op1()) === 0);
});
