...
after(function (spec, cpu) {
    ...
    [7, 6, 5, 4].forEach(function (pos, i) {
        ...
        if (!isNaN(reset)) {
            ...
            reset === 1 ? cpu._f |= flag : cpu._f &= ~flag;
        }
    });
...
inst('cp', function (spec, cpu) {
    ...
    if (this.op1 === cpu._a) return cpu._f |= 0x80;
    else if (this.op1 >> 8 > 0) return cpu._f |= 0x10;
    return cpu._f;
});
...
inst('dec', function (spec, cpu) {
    ...
    switch (spec.operands[0]) {
        case 'B': case 'C':
            cpu['_' + reg] -= 1;
            cpu['_' + reg] &= 0xFF;
            return cpu['_' + reg] === 0 ? cpu._f |= 0x80 : cpu._f;
        default: return;
    }
});
...
inst('jp', function (spec, cpu) {
    ...
    if (words == 2 && this.op1 == 1) return cpu._pc = this.op2;
    else if (words == 1) return cpu._pc = this.op1;
    return cpu._pc;
});
...
inst('ld', function (spec, cpu) {
    ...
    switch (spec.operands[0]) {
        case 'A': case 'B': case 'C':
            return cpu[reg] = this.op2;
        case 'SP':
            return cpu._sp = this.op2;
        case 'HL':
            cpu._h = this.op2 >> 8;
            return cpu._l = this.op2 & 0xFF;
        case '(HL)': case '(HL-)': case '(a16)':
            return cpu._mmu.writeWord(this.op1, this.op2);
        default: return;
    }
});
...
inst('rst', function (spec, cpu) {
    ...
    cpu._sp -= 2;
    cpu._mmu.writeWord(cpu._sp, cpu._pc);
