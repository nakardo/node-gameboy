...
var _flag = function (cpu, flag, set) {
    ...
    if (arguments.length == 2) return cpu.f & bit;
    return set ? cpu.f |= bit : cpu.f &= ~bit;
};
...
after(function (cpu, spec) {
    ...
    ['z', 'n', 'h', 'c'].forEach(function (flag, i) {
        ...
        if (!isNaN(value)) {
            debug('set flag %s=%d', flag.toUpperCase(), value);
            that.flag(flag, value);
        }
    });
...
inst('pop', function (cpu, op1) {
    ...
    console.log('pop', cpu[op1.operand].toString(16));
    return cpu.sp += 2;
});
...
inst('push', function (cpu, op1) {
    ...
    cpu.sp -= 2;
    console.log('push', cpu.mmu.writeWord(cpu.sp, op1()).toString(16));
...
inst('rla', function (cpu) {
    ...
    var shifted = cpu.a | this.flag('c');

    this.flag('c', shifted >> 8 > 0);
