...
var _flag = function (cpu, bit, set) {

    if (arguments.length === 1) return cpu.f & bit > 0;
    return set ? cpu.f |= bit : cpu &= ~(1 << bit);
};
...
after(function (spec, cpu) {
    ...
    [0x80, 0x40, 0x20, 0x10].forEach(function (bit, i) {
        var set = parseInt(spec.flags_znhc[i], 2);
        if (!isNaN(set)) {
            debug('set flag %s=%d', bit, set);
            that.flag(bit, set);
        }
    });
