...
class Cpu {
    ...
    _handleInterrupts () {
         ...
         */
        const ie = this._mmu.readWord(0xffff);
        if (ie == 0) return;
         ...
         */
         const intf = this._mmu.readWord(0xff0f);

         int('flags 0b%s', ief.toString(2));

         if (ie & intf & 1) { intf &= ~1; this._pc = 0x40; }
         else if (ie & intf & 2) { intf &= ~(1 << 1); this._pc = 0x48; }
         else if (ie & intf & 3) { intf &= ~(1 << 2); this._pc = 0x50; }
         else if (ie & intf & 4) { intf &= ~(1 << 3); this._pc = 0x58; }
         else if (ie & intf & 5) { intf &= ~(1 << 4); this._pc = 0x60; }
         else return;
