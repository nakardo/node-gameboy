...
Cpu.prototype.flag = function (flag, set) {
    ...
    if (arguments.length === 1) return bitutil.test(this.f, bit);
    ...
    return this.f = set
        ? bitutil.set(this.f, bit)
        : bitutil.reset(this.f, bit)
        ;
};
...
Cpu.prototype.serviceInterrupt = function (bit, intf) {
    ...
    this._ime = false;
    this.mmu.writeWord(IFLAGS, bitutil.reset(intf, bit));

    // Push to stack

    this._sp -= 2;
    this.mmu.writeWord(this._sp, this._pc);
...
Cpu.prototype.interrupt = function (bit) {
    ...
    this.mmu.writeWord(IFLAGS, this.mmu.readWord(IFLAGS) | 1 << bit);
};
...
Cpu.prototype.doInterrupts = function () {
    ...
    var intf = this.mmu.readWord(IFLAGS);

    if (intf === 0) return;

    var inte = this.mmu.readWord(ISWITCH);
