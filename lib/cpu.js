...
class Cpu {
    ...
    _handleInterrupts () {
        ...
        const flags = ie & intf;
        if (flags & 1) { intf &= ~1; pc = 0x40; }
        else if (flags & 2) { intf &= ~(1 << 1); pc = 0x48; }
        else if (flags & 4) { intf &= ~(1 << 2); pc = 0x50; }
        else if (flags & 8) { intf &= ~(1 << 3); pc = 0x58; }
        else if (flags & 0x10) { intf &= ~(1 << 4); pc = 0x60; }
        else return;
