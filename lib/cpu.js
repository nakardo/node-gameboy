...
class Cpu {
    ...
    powerOn () {
        debug('power on');
        this._loop = setInterval(() => this._step(), 1000 / 60);
    }
    ...
    _handleInterrupts () {
        ...
        else if (ie & intf & 4) { intf &= ~(1 << 2); addr = 0x50; }
        else if (ie & intf & 8) { intf &= ~(1 << 3); addr = 0x58; }
        else if (ie & intf & 0x10) { intf &= ~(1 << 4); addr = 0x60; }
        else return;
