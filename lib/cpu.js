...
class Cpu {
    ...
    _handleInterrupts () {
         ...
         */
         const ief = this._mmu.readWord(0xff0f);

         int('flags 0b%s', ief.toString(2));

         if (ie & ief & 1) { ief &= ~1; this._pc = 0x40; }
         else if (ie & ief & 2) { ief &= ~(1 << 1); this._pc = 0x48; }
         else if (ie & ief & 3) { ief &= ~(1 << 2); this._pc = 0x50; }
         else if (ie & ief & 4) { ief &= ~(1 << 3); this._pc = 0x58; }
         else if (ie & ief & 5) { ief &= ~(1 << 4); this._pc = 0x60; }
         else return;

         this._ime = 0;

         mmu.writeByte(0xff0f, ief);
    }
