...
const debugDivider = require('debug')('timer:divider');
const debugCounter = require('debug')('timer:counter');
...
class Timer {
    ...
    step (cycles) {
        ...
        if (this._divider > MAX_DIVIDER) {
            const div = this._mmu.readByte(DIV) + 1 & 0xff;

            debugDivider('update %s', div.toString(16));
            this._mmu.writeByte(DIV, div);
        ...
        if (this._t < this._clockSelect(tac)) return;
        ...
        debugCounter('update 0x%s', tima.toString(16));
