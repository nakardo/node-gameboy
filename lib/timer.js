...
class Timer {
    ...
    step (cycles) {
        ...
        if (this._divider > MAX_DIVIDER) {
            const counter = this._mmu.readByte(0xff04) + 1;

            divider('update %s', counter.toString(16));
            this._mmu.writeByte(0xff04, counter);
         ...
         * ALWAYS counting.
         */
        const control = this._mmu.readByte(0xff07);
        if (control & 0x4 == 0) {
            return;
        }
        ...
        if (this._t > this._clock(control)) {
            let counter = this._mmu.readByte(0xff05) + 1;
            if (counter == 0) {
                counter = this._mmu.readByte(0xff06);
                 ...
                 */
                 ...
                 this._mmu.writeByte(0xff0f, ief | 1 << 3);
            }
            timer('update 0x%s', counter.toString(16));
            this._mmu.writeByte(0xff05, counter);
