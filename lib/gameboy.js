...
class Gameboy {
    constructor (cart, bios = []) {
        this._cart = new Cart(cart);

        this._mmu = new Mmu(this._cart, bios);
        this._gpu = this._mmu.gpu = new Gpu(this._mmu);
        this._cpu = new Cpu(this._mmu, this._gpu);
        this._joypad = this._mmu.joypad = new Joypad(this._mmu);

        this._useBios = bios.length > 0;
    }
    ...
    powerOn () {
        ...
        if (!this._useBios) {
            debug('power up without bootstrap');
