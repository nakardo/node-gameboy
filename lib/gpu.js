...
const step = require('debug')('gpu:step');
const mode = require('debug')('gpu:mode');
const line = require('debug')('gpu:line');
...
class Gpu extends EventEmitter {
    ...
    step (cycles) {
        ...
        const pre = this.stat & 3;
        const cur = stat & 3;

        if (pre != cur) switch (cur) {
            case 1: this.stat |= 1 << 4;
            case 2: this.stat |= 1 << 5; break;
            case 0: this.stat |= 1 << 3; break;
        }

        mode('%d; ly=%d', cur, ly);

        if (this._t > 0) {
            if (pre != cur) this._requestInterrupt(INT_48);
            return;
        }
        ...
        if (ly < 144) this.drawLine(ly);

        // Next Scanline

        this.ly = ly < 153 ? ly + 1 : 0;
