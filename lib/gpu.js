...
class Screen extends EventEmitter {
    ...
    drawLine (line) {
        ...
        const mapOffset = 0x9800 + mapOffsetX + mapOffsetY;

        for (let tile = 0; tile < 20; tile++) {
            const mapTile = this._mmu.readByte(mapOffset + tile) * 16;
            const lineOffset = (mapY % 8) * 2;
            const data = this._mmu.readWord(0x8000 + mapTile + lineOffset);
            ...
            for (let tileX = 7; tileX > -1; tileX--) {
                ...
                image.data[0] = value[0];
                image.data[1] = value[1];
                image.data[2] = value[2];
                image.data[3] = 255;

                ctx.putImageData(image, posX, line);
            }
        }
    }
