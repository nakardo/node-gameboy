...
Lcd.prototype.step = function (cycles) {
    ...
    if (this._counter <= 0) {
        ...
        //  If the new scanline is 144 then this is the beginning of the vetical
        //  blank interupt so we need to request this interupt (remember that
        //  the vblank interupt is bit 0).
...
Lcd.prototype._updateStatus = function () {
    ...
    if (!this._isLcdEnabled()) {
        ...
        status &= 253; // 0b11111101

        return this._mmu.writeWord(LCDSTAT, status);
    }

    var currentLine = this._mmu.readWord(CURLINE);
