...
var onIoWriteEvent = function (addr, current, updated) {
    ...
    switch (addr) {
        case DIVIDER:
            this._mmu._io[DIVIDER - 0xFF00] = 0;
            break;
        case TIMECNT:
            var newfreq = this._getClockFreq();
            if (this._getClockFreq(current) !== newfreq) {
...
Timer.prototype.powerOn = function () {
    ...
    this._counter = this._getClockFreq();
};
...
Timer.prototype._updateDivider = function (cycles) {
...
Timer.prototype._isTimerEnabled = function () {
    return bitutil.test(this._mmu.readByte(TIMCONT), 4);
};

Timer.prototype._getClockFreq = function (value) {
