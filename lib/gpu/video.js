...
class Video extends Serializable() {
    constructor () {
        super();
        this._ram = new Array(2).fill().map(() => new Uint8Array(0x2000));
    ...
    fromJSON (obj) {
        ...
        this._ram = obj._ram.map(v => new Uint8Array(v));
        this._oam = new Uint8Array(obj._oam);

        // TODO(nakardo): do we need to rerun this? requires to map also VRAM
        // bank changes to work.
    ...
    readByte (addr) {
        switch (addr >> 12) {
            case 0x8: case 0x9:
                return this._ram[this.ramBank][addr & 0x1fff];
            case 0xf:
        ...
        switch (addr >> 12) {
            case 0x8: case 0x9:
                if (addr > 0x97ff) {
                    ...
                    const res = this._sumBytes(
                        this._ram[this.ramBank][pos - 1],
                        val
                    );
                    this.tiles[pos >> 4][pos >> 1 & 7] = res;
                }
                return this._ram[this.ramBank][addr & 0x1fff] = val;
            case 0xf:
                if (addr > 0xfe9f) break;
                if (addr > 0xfdff) {
                    const pos = addr & 0xff;
