...
$cb[0x06] = ['RLC (HL)', (cpu) => {
    const v = mmu.readByte(cpu.hl) << 1;

    cpu.f = 0;
    if (!(v & 0xff)) cpu.f |= 1 << 7;
    if (v & 0x100) cpu.f |= 1 << 4;

    mmu.writeByte(cpu.hl, v | v >> 8);
    cpu.pc += 2;
