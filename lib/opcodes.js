...
$[0x86] = ['ADD A,(HL)', (cpu, mmu) => {
    const v = cpu.a + mmu.readByte(cpu.hl);
    cpu.a = v;
...
$[0xc6] = ['ADD A,#', (cpu, mmu) => {
    const v = cpu.a + mmu.readByte(cpu.pc + 1);
    cpu.a = v;
...
const ADC_A_n = (n, cpu) => {
    const nc = cpu[n] + cpu.f >> 4 & 1;
    const v = cpu.a + nc;
    cpu.a = v;

    cpu.f = 0;
    if (!cpu.a) cpu.f |= 1 << 7;
    if ((cpu.a & 0xf + nc & 0xf) > 0xf) cpu.f |= 1 << 5;
    if (v > 0xff) cpu.f |= 1 << 4;
...
$[0x8e] = ['ADC A,(HL)', (cpu, mmu) => {
    const nc = mmu.readByte(cpu.hl) + cpu.f >> 4 & 1;
    const v = cpu.a + nc;
    cpu.a = v;
...
$[0xce] = ['ADC A,#', (cpu, mmu) => {
    const nc = mmu.readByte(cpu.pc + 1) + cpu.f >> 4 & 1;
    const v = cpu.a + nc;
    cpu.a = v;
...
$[0x9f] = ['SBC A,A', (...args) => SUB_A_n('a', ...args)];
$[0x98] = ['SBC A,B', (...args) => SUB_A_n('b', ...args)];
$[0x99] = ['SBC A,C', (...args) => SUB_A_n('c', ...args)];
$[0x9a] = ['SBC A,D', (...args) => SUB_A_n('d', ...args)];
$[0x9b] = ['SBC A,E', (...args) => SUB_A_n('e', ...args)];
$[0x9c] = ['SBC A,H', (...args) => SUB_A_n('h', ...args)];
$[0x9d] = ['SBC A,L', (...args) => SUB_A_n('l', ...args)];
