 ...
 *
 * Notes:
 * The values of the carry and halfcarry are computed on the low 8bits.
 */

$[0xf8] = ['LD HL,SP+n', (cpu, mmu) => {
    const n = mmu.readByte(cpu.pc + 1);
    const r = cpu.sp + n.signed();
...
const ADD_A_n = (n) => function (cpu) {
    ...
    if ((cpu.a ^ cpu[n] ^ r) & 0x10 != 0) cpu.f |= FLAG_H;
...
const SBC_A_n = (n) => function (cpu, mmu) {
    const cy = cpu.f >> 4 & 1;
    const r = cpu.a - cpu[n] - cy;
 ...
 *
 * Notes:
 * Various sources disagree on whether the Z flag should be modified by the
 * operation. After some research I'm pretty sure it shouldn't be modified (the
 * Z80 processor for instance does not modify Z).
 */
...
const RLC_n = (n) => function (cpu) {
    const r = cpu[n] << 1 | cpu[n] >> 7;

    cpu.f = 0;
    if (r == 0) cpu.f |= FLAG_Z;
    if (cpu[n] & 0x80 != 0) cpu.f |= FLAG_C;
...
const JP_cc_nn = function (cc, cpu, mmu) {
    if (cc) {
        cpu.pc = mmu.readWord(cpu.pc + 1);
        return 16;
    }
...
$[0xc2] = ['JP NZ,nn', (cpu, mmu) => JP_cc_nn((cpu.f & FLAG_Z) == 0, cpu, mmu)];
$[0xd2] = ['JP NC,nn', (cpu, mmu) => JP_cc_nn((cpu.f & FLAG_C) == 0, cpu, mmu)];

$[0xca] = ['JP Z,nn', (cpu, mmu) => JP_cc_nn(cpu.f & FLAG_Z, cpu, mmu)];
$[0xda] = ['JP C,nn', (cpu, mmu) => JP_cc_nn(cpu.f & FLAG_C, cpu, mmu)];
...
const JR_cc_n = function (cc, cpu, mmu) {
    if (cc) {
        const n = mmu.readByte(cpu.pc + 1);
        cpu.pc += 2 + n.signed();
...
$[0x20] = ['JR NZ,n', (cpu, mmu) => JR_cc_n((cpu.f & FLAG_Z) == 0, cpu, mmu)];
$[0x30] = ['JR NC,n', (cpu, mmu) => JR_cc_n((cpu.f & FLAG_C) == 0, cpu, mmu)];

$[0x28] = ['JR Z,n', (cpu, mmu) => JR_cc_n(cpu.f & FLAG_Z, cpu, mmu)];
$[0x38] = ['JR C,n', (cpu, mmu) => JR_cc_n(cpu.f & FLAG_C, cpu, mmu)];
...
const CALL_cc_nn = function (cc, cpu, mmu) {
    if (cc) {
        cpu.sp -= 2;
        mmu.writeWord(cpu.sp, cpu.pc + 3);
        cpu.pc = mmu.readWord(cpu.pc + 1);
...
$[0xc4] = ['CALL NZ,nn', (cpu, mmu) => {
    return CALL_cc_nn((cpu.f & FLAG_Z) == 0, cpu, mmu);
}];
$[0xd4] = ['CALL NC,nn', (cpu, mmu) => {
    return CALL_cc_nn((cpu.f & FLAG_C) == 0, cpu, mmu);
}];

$[0xcc] = ['CALL Z,nn', (cpu, mmu) => CALL_cc_nn(cpu.f & FLAG_Z, cpu, mmu)];
$[0xdc] = ['CALL C,nn', (cpu, mmu) => CALL_cc_nn(cpu.f & FLAG_C, cpu, mmu)];
...
const RET_cc = function (cc, cpu, mmu) {
    if (cc) {
        cpu.pc = mmu.readWord(cpu.sp);
        cpu.sp += 2;
...
$[0xc0] = ['RET NZ', (cpu, mmu) => RET_cc((cpu.f & FLAG_Z) == 0, cpu, mmu)];
$[0xd0] = ['RET NC', (cpu, mmu) => RET_cc((cpu.f & FLAG_C) == 0, cpu, mmu)];

$[0xc8] = ['RET Z', (cpu, mmu) => RET_cc(cpu.f & FLAG_Z, cpu, mmu)];
$[0xd8] = ['RET C', (cpu, mmu) => RET_cc(cpu.f & FLAG_C, cpu, mmu)];
