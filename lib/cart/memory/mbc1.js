...
class MBC1 extends Serializable({ exclude: ['_rom'] }) {
    constructor (data, romSize, ramSize) {
        ...
        debug('rom', this._rom, ROM_SIZE[romSize]);
        debug('ram', this._ram, RAM_SIZE[ramSize]);
    }
    ...
    readByte (addr) {
        switch (addr >> 12) {
            ...
            case 0x4: case 0x5:
            case 0x6: case 0x7: {
                let bank = this._lbank;
                if (this._mode == 0) bank |= this._hbank << 5;
    ...
    writeByte (addr, val) {
        switch (addr >> 12) {
            ...
            case 0xa: case 0xb:
                ...
                return wrap(this._ram, this._hbank)[addr & 0x1fff] = val;
        }
