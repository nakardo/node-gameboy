...
function isMulticart (data, romSize) {
    if (ROM_SIZE[romSize] != 64) {
        return false;
    }

    let count = 0;
    for (let i = 0; i < data.length; i += 0x4000) {
        const offset = 0x104 + i;
        const logo = data.slice(offset, offset + NINTENDO_LOGO.length);

        let match = true;
        for (let j = 0; j < logo.length; j++) {
            match = match && data[offset + j] == NINTENDO_LOGO[j];
            if (!match) break;
        }
        if (match) count++;
    }
...
class MBC1 extends Serializable({ exclude: ['_rom'] }) {
    constructor (data, romSize, ramSize) {
        ...
        this._isMulticart = isMulticart(data, romSize);
        this._mode = 0;
